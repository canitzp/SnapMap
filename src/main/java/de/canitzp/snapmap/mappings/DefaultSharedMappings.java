...
/**
 * This is the default SharedMappings class out of DynamicMappings, but with changes to avoid all errors
 *
 * @author FyberOptic
 */
public class DefaultSharedMappings extends CustomMappingBase {
    ...
    public boolean getBlockClass() {
        ...
        for (String key : classes.keySet()) {
            if (classes.get(key) > mostCount) {
                mostClass = key;
                mostCount = classes.get(key);
            }
        }
    ...
    public boolean processIBlockStateClass() {
        ...
        if (iBlockState.interfaces.size() == 2) {
            ...
            if (cn1 != null && cn2 != null) {
                ...
                if (cn1Size > 20 && cn2Size < cn1Size) {
                    wrapperName = cn1.name;
                    eventsName = cn2.name;
                }
                if (cn2Size > 20 && cn1Size < cn2Size) {
                    wrapperName = cn2.name;
                    eventsName = cn1.name;
                }
    ...
    public boolean processBlockPosClass() {
        ...
        for (int n = 0; n < 6; n++) {
            if (facings[n] == null) return false;
        }
    ...
    public boolean processVec3iClass() {
        ...
        for (MethodNode method : methods) {
            if (matchOpcodeSequence(method.instructions.getFirst(), Opcodes.ALOAD, Opcodes.GETFIELD, Opcodes.IRETURN)) {
                ...
                if (fn.name.equals(x))
                    addMethodMapping("net/minecraft/util/Vec3i getX ()I", vec3i.name + " " + method.name + " ()I");
                else if (fn.name.equals(y))
                    addMethodMapping("net/minecraft/util/Vec3i getY ()I", vec3i.name + " " + method.name + " ()I");
                else if (fn.name.equals(z))
                    addMethodMapping("net/minecraft/util/Vec3i getZ ()I", vec3i.name + " " + method.name + " ()I");
            }
        }
    ...
    public boolean getEntityItemClass() {
        ...
        for (MethodNode method : blockClass.methods) {
            ...
            if (arguments[0].getSort() != Type.OBJECT || arguments[1].getSort() != Type.OBJECT || arguments[2].getSort() != Type.OBJECT)
                continue;
            // Make sure arg0 is World and arg1 is BlockPos
            if (!arguments[0].getClassName().equals(worldClass.name) || !arguments[1].getClassName().equals(blockPosClass.name))
                continue;

            boolean foundString = false;
            for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
                if (!foundString && isLdcWithString(insn, "doTileDrops")) {
                    foundString = true;
                    continue;
                }
    ...
    public boolean processBlockSlabClass() {
        ...
        if (!MeddleUtil.notNull(block, blockSlab, material, iBlockState, iBlockAccess, blockPos, aabb, mapColor))
            return false;
        ...
        if (methods.size() == 3 && isOpaqueCube != null) {
            for (Iterator<MethodNode> it = methods.iterator(); it.hasNext(); ) {
                ...
                if (!matchOpcodeSequence(method.instructions.getFirst(), Opcodes.ALOAD, Opcodes.INVOKEVIRTUAL, Opcodes.IRETURN))
                    it.remove();
            }
            if (methods.size() == 1) {
    ...
    public boolean processTileEntityClass() {
        ...
        if (methods.size() == 2) {
            ...
            for (Iterator<MethodNode> it = methods.iterator(); it.hasNext(); ) {
                if ((it.next().access & Opcodes.ACC_PRIVATE) != 0) it.remove();
            }
    ...
    public boolean processTileEntityChestClass() {
        ...
        ClassNode iInventory = getClassNodeFromMapping("net/minecraft/inventory/IInventory");
        if (!MeddleUtil.notNull(tileEntity, tileEntityChest, inventoryPlayer, entityPlayer, container, iInventory))
            return false;
    ...
    public boolean getEntityPlayerClass() {
        ...
        for (MethodNode method : serverClass.methods) {
            ...
            if (!args[0].getClassName().equals(worldClass.name) || !args[1].getClassName().equals(blockPosClass.name))
                continue;
    ...
    public boolean processEntityClass() {
        ...
        for (Iterator<MethodNode> it = methods.iterator(); it.hasNext(); ) {
            ...
            if (!mn.owner.equals(entity.name) || !mn.name.equals(getFlag.name) || !mn.desc.equals(getFlag.desc))
                it.remove();
        }
        if (methods.size() == 1) {
    ...
    public boolean processBlockTNTClass() {
        ...
        ClassNode entityLivingBase = getClassNodeFromMapping("net/minecraft/entity/EntityLivingBase");
        ClassNode sound = getClassNodeFromMapping("net/minecraft/util/Sound");
        if (!MeddleUtil.notNull(block, tnt, world, blockPos, iBlockState, propertyBool, entity, entityLivingBase, sound))
            return false;
        ...
        if (clinit != null) {
            ...
            for (Iterator<MethodInsnNode> it = nodes.iterator(); it.hasNext(); ) {
                MethodInsnNode mn = it.next();
                if (mn.owner.equals(propertyBool.name) && mn.desc.equals("(Ljava/lang/String;)L" + propertyBool.name + ";"))
                    continue;
                it.remove();
            }
            if (nodes.size() == 1) {
    ...
    public boolean processBlockAirClass() {
        ...
        ClassNode aabb = getClassNodeFromMapping("net/minecraft/util/AxisAlignedBB");
        ClassNode iBlockAccess = getClassNodeFromMapping("net/minecraft/world/IBlockAccess");
        if (!MeddleUtil.notNull(block, blockAir, world, blockPos, iBlockState, material, aabb, iBlockAccess))
            return false;
    ...
    public boolean processBlockLog() {
        ...
        if (methods.size() == 1) {
            ...
            for (Iterator<MethodInsnNode> it = methodNodes.iterator(); it.hasNext(); ) {
                MethodInsnNode mn = it.next();
                if (!(mn.owner.equals(world.name) && mn.desc.equals("(L" + blockPos.name + ";L" + blockPos.name + ";)Z")))
                    it.remove();
            }
            ...
            for (Iterator<MethodInsnNode> it = methodNodes.iterator(); it.hasNext(); ) {
                MethodInsnNode mn = it.next();
                if (!(mn.owner.equals(blockPos.name) && mn.desc.equals(assembleDescriptor("(", blockPos, blockPos, ")Ljava/lang/Iterable;"))))
                    it.remove();
            }
            if (methodNodes.size() == 1) {
            ...
            if (fields.size() == 4) {
                for (FieldNode field : fields) {
                    if (field.name.equals(slotIndex))
                        ...
                        addFieldMapping("net/minecraft/inventory/Slot slotNumber I", slot.name + " " + field.name + " I");
                }
            }
        }
    ...
    public boolean processItemBowClass() {
        ...
        if (!MeddleUtil.notNull(item, itemBow, itemStack, world, entityPlayer, entityLivingBase, hand, sound))
            return false;
    ...
    public boolean processEnumActionClass() {
        ...
        for (AbstractInsnNode insn = init.instructions.getFirst(); insn != null; insn = insn.getNext()) {
            ...
            if (insn.getOpcode() == Opcodes.PUTSTATIC) {
                FieldInsnNode fn = (FieldInsnNode) insn;
                if (fn.owner.equals(enumAction.name) && fn.desc.equals("L" + enumAction.name + ";"))
                    fields.add(fn.name);
            }
        }
    ...
    public boolean processBlockClass() {
        ...
        if (materialClass != null && mapColorClass != null)
            methods = getMatchingMethods(block, "<init>", "(L" + materialClass + ";L" + mapColorClass + ";)V");
        if (methods.size() == 1) {
            for (AbstractInsnNode insn = methods.get(0).instructions.getFirst(); insn != null; insn = insn.getNext()) {
                if (insn.getOpcode() != Opcodes.INVOKEVIRTUAL) continue;
                MethodInsnNode mn = (MethodInsnNode) insn;
        ...
        methods.clear();
        for (MethodNode method : block.methods) {
            if (!method.desc.startsWith("(L" + iBlockState.name + ";L" + world.name + ";L" + blockPos.name + ";L"))
                continue;
    ...
    public boolean processItemClass() {
        ...
        ClassNode entityLivingBase = getClassNodeFromMapping("net/minecraft/entity/EntityLivingBase");
        ClassNode iBlockState = getClassNodeFromMapping("net/minecraft/block/state/IBlockState");
        if (!MeddleUtil.notNull(item, block, itemStack, world, blockPos, entityPlayer, entityLivingBase, iBlockState))
            return false;
        ...
        for (MethodNode method : item.methods) {
            if (!checkMethodParameters(method, Type.OBJECT, Type.OBJECT, Type.OBJECT, Type.OBJECT, Type.OBJECT, Type.FLOAT, Type.FLOAT, Type.FLOAT))
                continue;
        ...
        for (FieldNode field : item.fields) {
            if ((field.access & (Opcodes.ACC_FINAL | Opcodes.ACC_STATIC)) != (Opcodes.ACC_FINAL + Opcodes.ACC_STATIC))
                continue;
    ...
    public boolean processEnumFacingClass() {
        ...
        for (FieldNode fn : enumFacing.fields) {
            if (fn.desc.startsWith("L" + enumFacing.name + "$"))
        ...
        if (innerClasses.size() == 2) {
            ...
            for (String s : innerClasses) {
                if (axisDirection_name == null && searchConstantPoolForStrings(s, "POSITIVE", "NEGATIVE"))
            ...
            if (inits.size() == 7) {
                for (AbstractInsnNode[] nodes : inits) {
                    ...
                    switch (var) {
                        ...
                        case 5:
                            if (fn.desc.equals("I"))
                                addFieldMapping("net/minecraft/util/EnumFacing horizontalIndex I", fn);
                        ...
                        case 6:
                            if (fn.desc.equals("Ljava/lang/String;"))
                                addFieldMapping("net/minecraft/util/EnumFacing name Ljava/lang/String;", fn);
                        ...
                        case 7:
                            if (fn.desc.equals("L" + axisDirection.name + ";"))
                                addFieldMapping("net/minecraft/util/EnumFacing axisDirection Lnet/minecraft/util/EnumFacing$AxisDirection;", fn);
                        ...
                        case 8:
                            if (fn.desc.equals("L" + axis.name + ";"))
                                addFieldMapping("net/minecraft/util/EnumFacing axis Lnet/minecraft/util/EnumFacing$Axis;", fn);
                        ...
                        case 9:
                            if (fn.desc.equals("L" + vec3i.name + ";"))
                                addFieldMapping("net/minecraft/util/EnumFacing directionVec Lnet/minecraft/util/Vec3i;", fn);
    ...
    public boolean processWorldClass() {
        ...
        ClassNode entity = getClassNodeFromMapping("net/minecraft/entity/Entity");
        ClassNode sound = getClassNodeFromMapping("net/minecraft/util/Sound");
        if (!MeddleUtil.notNull(world, blockPos, iBlockState, entityPlayer, block, tileEntity, aabb, entity, sound))
            return false;
    ...
    public boolean processBlockChestClass() {
        ...
        ClassNode iBlockState = getClassNodeFromMapping("net/minecraft/block/state/IBlockState");
        ClassNode iBlockWrapper = getClassNodeFromMapping("net/minecraft/block/state/IBlockWrapper");
        if (!MeddleUtil.notNull(blockChest, entityPlayer, world, blockPos, ocelot, iBlockState, iBlockWrapper))
            return false;
    ...
    public boolean processEntityPlayerClass() {
        ...
        for (MethodNode method : entityPlayer.methods) {
            Type t = Type.getMethodType(method.desc);
            Type[] args = t.getArgumentTypes();
            if (args.length != 1 || args[0].getSort() != Type.OBJECT || t.getReturnType().getSort() != Type.VOID)
                continue;
    ...
    public boolean getNBTClasses() {
        ...
        for (Integer key : nbtClassesMap.keySet()) {
            ...
            switch (key) {
                case 0:
                    addClassMapping("net/minecraft/nbt/NBTTagEnd", className);
                    continue;
                case 1:
                    addClassMapping("net/minecraft/nbt/NBTTagByte", className);
                    continue;
                case 2:
                    addClassMapping("net/minecraft/nbt/NBTTagShort", className);
                    continue;
                case 3:
                    addClassMapping("net/minecraft/nbt/NBTTagInt", className);
                    continue;
                case 4:
                    addClassMapping("net/minecraft/nbt/NBTTagLong", className);
                    continue;
                case 5:
                    addClassMapping("net/minecraft/nbt/NBTTagFloat", className);
                    continue;
                case 6:
                    addClassMapping("net/minecraft/nbt/NBTTagDouble", className);
                    continue;
                case 7:
                    addClassMapping("net/minecraft/nbt/NBTTagByteArray", className);
                    continue;
                case 8:
                    addClassMapping("net/minecraft/nbt/NBTTagString", className);
                    continue;
                case 9:
                    addClassMapping("net/minecraft/nbt/NBTTagList", className);
                    continue;
        ...
        for (MethodNode method : methods) {
            ...
            for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
                if (insn.getOpcode() == Opcodes.GETFIELD) {
                    ...
                    if (fn.owner.equals(inventoryBasic.name) && fn.desc.equals("L" + mcList.name + ";"))
                        hasArray = true;
                }
            }
        ...
        // A hack since 15w34c duplicated the method for seemingly no reason
            ...
            if (mn1.instructions.size() == mn2.instructions.size()) {
                ...
                for (int n = 0; n < size; n++) {
                    if (mn1.instructions.get(n).getOpcode() != mn2.instructions.get(n).getOpcode()) {
                        nomatch = true;
                        break;
                    }
                }
    ...
    public boolean processItemStackClass() {
        ...
        for (Iterator<MethodNode> iterator = methods.iterator(); iterator.hasNext(); ) {
            ...
            if (!correctSequence || !correctField) {
                iterator.remove();
                continue;
            }
        }
        ...
        if (methods.size() == 3) {
            for (MethodNode method : methods) {
                ...
                for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
                    if (isLdcWithString(insn, "display")) {
                        isDisplayName = true;
                        break;
                    }
                }
    ...
    public boolean processItemStackClass2() {
        ...
        if (getMaxDamage != null) {
            ...
            for (Iterator<MethodNode> it = methods.iterator(); it.hasNext(); ) {
                ...
                boolean hasIt = false;
                for (MethodInsnNode mn : nodes) {
                    if (mn.owner.equals(item.name) && mn.name.equals(getMaxDamage.name) && mn.desc.equals("()I"))
                        hasIt = true;
                }
                if (!hasIt) it.remove();
            }
    ...
    @Mapping(
            depends = {ENTITY_SHEEP},
            provides = {CRAFTING_MANAGER, ENTITY_ANIMAL, ENUM_DYE_COLOR}
            )
    public void getCraftingManagerClass() {
        ClassNode sheepClass = getClassNodeFromMapping(ENTITY_SHEEP);
        if(MeddleUtil.notNull(sheepClass)){
            List<MethodNode> methods = getMatchingMethods(sheepClass, Opcodes.ACC_PRIVATE, Type.OBJECT, Type.OBJECT, Type.OBJECT);
            if(methods.size() == 1){
                addClassMapping(ENUM_DYE_COLOR, Type.getReturnType(methods.get(0).desc).getClassName());
                addClassMapping(ENTITY_ANIMAL, Type.getArgumentTypes(methods.get(0).desc)[0].getClassName());
                for(AbstractInsnNode node : methods.get(0).instructions.toArray()){
                    if(node.getOpcode() == Opcodes.INVOKESTATIC){
                        MethodInsnNode methodNode = (MethodInsnNode) node;
                        addClassMapping(CRAFTING_MANAGER, methodNode.owner);
                        break;
                    }
                }
            }
    ...
    */
    ...
    public boolean processEntityLivingBase() {
        ...
        for (Iterator<MethodNode> it = methods.iterator(); it.hasNext(); ) {
            MethodNode method = it.next();
            if (matchOpcodeSequence(method.instructions.getFirst(), Opcodes.ALOAD, Opcodes.GETFIELD, Opcodes.ARETURN))
                it.remove();
        }
        ...
        if (methods.size() == 1) {
            ...
            for (Iterator<MethodInsnNode> it = mnodes.iterator(); it.hasNext(); ) {
                ...
                if (getMethodNode(entity, "--- " + mn.name + " (DDD)V") == null) {
                    it.remove();
                    continue;
                }
    ...
    public boolean processBlockClass4() {
        ...
        ClassNode blockPos = getClassNodeFromMapping("net/minecraft/util/BlockPos");
        ClassNode aabb = getClassNodeFromMapping("net/minecraft/util/AxisAlignedBB");
        if (!MeddleUtil.notNull(block, iBlockState, iBlockWrapper, material, resourceLocation, world, blockPos, aabb))
            return false;
    ...
    public void findNonNullList(){
        ...
        if(MeddleUtil.notNull(itemClass, creativeTabsClass)){
            List<MethodNode> methods = getMatchingMethods(itemClass, Opcodes.ACC_PUBLIC, Type.VOID, Type.OBJECT, Type.OBJECT, Type.OBJECT);
            for(MethodNode method : methods){
                if(Objects.equals(Type.getArgumentTypes(method.desc)[0].getClassName(), itemClass.name)){
            ...
            if(methods.size() == 1){
                ...
                for(AbstractInsnNode node : methods.get(0).instructions.toArray()){
                    if(node.getOpcode() == Opcodes.INVOKESTATIC && node.getNext().getOpcode() != Opcodes.ASTORE){
                        MethodInsnNode methodNode = (MethodInsnNode) node;
                        if(!second){
                            addClassMapping(SHAPED_RECIPE, methodNode.owner);
                            second = true;
                        } else {
                            addClassMapping(SHAPELESS_RECIPE, methodNode.owner);
                            break;
                        }
                    }
                }
